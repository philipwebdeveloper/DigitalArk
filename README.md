...
- Theme: near-black background with a deep-blue-to-cyan accent, matched directly to your logo (`assets/logo_original.jpg` sampled colors: `#0057F7` deep blue → `#1EE4FF` bright cyan).
- Logo: `assets/logo-icon.png` (icon only, transparent background) is used in the nav bar and as the favicon. `assets/logo-full.png` (icon + wordmark + "Your Partner in Digital Success" tagline, transparent background) is used in the footer.
- Fonts: Space Grotesk (headings), Inter (body text), JetBrains Mono (labels), loaded from Google Fonts.
- Animations: scroll-reveal fades, hover lifts on cards, an animated pulsing signature dot in the hero, and a smooth mobile menu. Respects users who prefer reduced motion.
- All accent colors are controlled by two variables in `css/style.css`: `--gold` (base blue) and `--gold-bright` (bright cyan) — despite the variable names, these now hold your blue/cyan brand colors. Change them there to retint the whole site.
